...
import (
	...
	"go.seankhliao.com/fin/proto/fin"
	"go.seankhliao.com/fin/run"
	"go.seankhliao.com/fin/store"
)
...
func summarize(m *fin.Month) MonthSummary {
	delta := make(map[fin.Category]int64)
...
func group(name string, first, last fin.Category, delta map[fin.Category]int64) GroupSummary {
	s := GroupSummary{
		Name: name,
	}
